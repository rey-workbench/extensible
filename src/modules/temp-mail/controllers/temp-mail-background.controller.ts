...
export class TempMailBackgroundController {
  public static readonly contextType: ExecutionContext = "background";
  public static readonly inject = [TempMailService, MessageRouterService, EventBusService] as const;
  ...
  private _registerRoutes(): void {
    ...
    this.router.subscribe(
      ...
      async (payload: { messageId: string | number }) => {
  ...
  private async _fillCurrentActiveTab(targetTabId: number | null = null): Promise<boolean> {
    ...
    try {
      if (targetTabId) {
        await this.router.sendToTab(targetTabId, TEMPMAIL_ACTIONS.AUTOFILL_EMAIL, {
          email: emailAddress,
        });
      } else {
        await this.router.sendToActiveTab(TEMPMAIL_ACTIONS.AUTOFILL_EMAIL, {
          email: emailAddress,
        });
      }
      return true;
